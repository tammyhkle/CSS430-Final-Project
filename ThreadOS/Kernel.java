      ...
      switch (irq) {
         case INTERRUPT_SOFTWARE: // System calls
            switch (cmd) {
               ...
               case WAIT:
                  myTcb = scheduler.getMyTcb(); // get the current thread id
                  int tid = myTcb.getTid();

                  // sleep in waitQueue, condition = this thread id
                  return waitQueue.enqueueAndSleep(tid); // return a child thread id who woke me up
               case EXIT:
                  myTcb = scheduler.getMyTcb();
                  tid = myTcb.getTid(); // child ID
                  int pid = myTcb.getPid(); // get the current thread's parent id
                  waitQueue.dequeueAndWakeup(pid, tid); // wakeup thread, condition = thread's parent id
                  scheduler.deleteThread(); // tell the Scheduler to delete the current thread
                  return OK;
